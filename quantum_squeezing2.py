...
class QuantumSqueezingSideBySide(Scene):
    def construct(self):
        ...
        # Labels for Axes
        left_labels = left_axes.get_axis_labels(x_label="q_a", y_label="p_a")
        right_labels = right_axes.get_axis_labels(x_label="q_b", y_label="p_b")
        ...
        left_vacuum = Circle(
            radius=0.8, color=BLUE, stroke_width=3
        ).move_to(left_axes.c2p(0, 0))
        left_quantum = Circle(
            radius=1.5, color=RED, stroke_width=3, fill_color=RED, fill_opacity=0.2
        ).move_to(left_axes.c2p(0, 0))

        right_vacuum = Circle(
            radius=0.8, color=BLUE, stroke_width=3
        ).move_to(right_axes.c2p(0, 0))
        right_quantum = Circle(
            radius=1.5, color=RED, stroke_width=3
        ).move_to(right_axes.c2p(0, 0))

        # Create Vacuum Noise for Both
        self.play(Create(left_vacuum), Create(right_vacuum))
        self.wait(0.5)

        # Create Quantum Noise (Red Circle) for Left Plot and Squeeze into Ellipse for Right Plot
        ...
        self.play(Create(left_quantum), Transform(right_quantum, squeezed_ellipse))
        self.wait(1)
